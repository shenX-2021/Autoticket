...
from WaitPageLoad import waitPageLoad
...
class Concert(object):
    ...
    def choose_ticket_1_v2(self):  # for type 1, i.e., detail.damai.cn
        self.time_start = time()
        print("###开始进行日期及票价选择###")
        ...
        while True:
            try:
                ...
                if buybutton_text == "即将开抢" or buybutton_text == "即将开售":
                    self.status = 2
                    self.driver.refresh()
                    with waitPageLoad(self.driver, '等待页面刷新超时', 60):
                        print('---尚未开售，等待页面刷新---')
                    print('---开售了，页面刷新---')
                
                buybutton_text = buybutton.text
                print('按钮文字:', buybutton_text)

                if buybutton_text == "立即预订":
                    with waitPageLoad(self.driver, '等待页面刷新超时', 1):
                        print('---点击购买按钮，等待进入确认订单页---')
                        add_ticket()
                        buybutton.click()
                        self.status = 3
                    ...
                    # buybutton.click()
                    self.status = 5
                    with waitPageLoad(self.driver, 300):
                        print("###请自行选择位置和票价###")
        ...
        while True:
            try:
                if self.status in [3, 4]:
                    print('###开始确认订单###')
                    self.confirm_url = self.driver.current_url
                    self.driver.get(self.confirm_url)
                    # newwindow = 'window.open("' + self.confirm_url + '")'
